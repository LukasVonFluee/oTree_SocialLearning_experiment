...
# Methods: This is a lab study conducted in collaboration with the Decision 
# Science Laboratory at the ETH Zürich (\url{https://www.descil.ethz.ch/}). 
...
# Custom label function to format x-axis labels in the odds ratio plots (handling missing or non-numeric values and rounding values)
...
df_original = read.csv("zurich_all_sessions.csv")

# Note, before the 7th session, a session was accidentally created. This created
# an empty data table which is now included in the data that I downloaded from 
# the ETH server, because they accumulated the sessions so that I was not able 
# to download the data from the sessions individually. I.e., the empty data 
# table that was accidentally created, with the session code "ojhuqte4", has to 
# be removed for data analysis:

df_original <- subset(df_original, df_original$session.code != 'ojhuqte4')
...
df_soc_learn_4round_zurich <- subset(df_soc_learn, round_number %% number_rounds_per_block == 0)

nrow(df_soc_learn_4round_zurich)

length(unique(df_soc_learn_4round_zurich$participant_ID))
...
cat("did IL choose optimally in final periods", indFinalRobustCapture, file = "opt_dem_zurich.txt", sep = "n", fill = TRUE, append = TRUE)
...
freq_optimal_urn_triangle # = 0.925
...
freq_optimal_urn_square # = 0.93125
...
overall_frequency_chosen_optimal # = 0.928125 -> this is close to the 95.5% that Charles et al. (2016) find
...
df_soc_learn_4round_zurich$ILNoCorrTriangle <- rep(0,nrow(df_soc_learn_4round_zurich))
df_soc_learn_4round_zurich$ILNoCorrSquare <- rep(0,nrow(df_soc_learn_4round_zurich))

for (j in 1:nrow(df_soc_learn_4round_zurich))
{
  thisSessRow <- df_soc_learn_4round_zurich[j, ]
  thisSess <- thisSessRow$trialID
  useThisSess <- which(sessionArray == thisSess)
  df_soc_learn_4round_zurich$ILNoCorrTriangle[j] <- sessionMatrix[useThisSess,"triangle_dem"]
}
...
for (j in 1:nrow(df_soc_learn_4round_zurich))
{
  ...
  thisSess <- thisSessRow$trialID
  useThisSess <- which(sessionArray == thisSess)
  df_soc_learn_4round_zurich$ILNoCorrSquare[j] <- sessionMatrix[useThisSess,"square_dem"]
}
...
df_soc_learn_4round_zurich$propIndCorrCenteredTriangle <- (df_soc_learn_4round_zurich$ILNoCorrTriangle / number_ind_learn_per_group) - 0.5
df_soc_learn_4round_zurich$propIndCorrCenteredSquare <- (df_soc_learn_4round_zurich$ILNoCorrSquare / number_ind_learn_per_group) - 0.5
...
df_soc_learn_4round_zurich$propIndCorrCentered <- NA

# Triangle

subset_triangle_soc <- df_soc_learn_4round_zurich[df_soc_learn_4round_zurich$group_assignment=="triangle",]
...
subset_square_soc <- df_soc_learn_4round_zurich[df_soc_learn_4round_zurich$group_assignment=="square",]
...
df_soc_learn_4round_zurich[df_soc_learn_4round_zurich$group_assignment=="triangle",] <- subset_triangle_soc
df_soc_learn_4round_zurich[df_soc_learn_4round_zurich$group_assignment=="square",] <- subset_square_soc
...
df_soc_learn_4round_zurich$propIndRightCentered <- NA
...
df_soc_learn_4round_zurich$similar_ingroup <- ifelse(df_soc_learn_4round_zurich$similar_ingroup==0.9, 1,0)
...
df_soc_learn_4round_zurich$gender <- factor(df_soc_learn_4round_zurich$gender, levels = c("Woman", "Man", "Other"), labels = c(0, 1, 2))

# Code study_subject as factor levels (still have to do this):

all_study_subjects <- as.list(unique(df_soc_learn_4round_zurich$study_subject))


# Function to map study fields to the three main categories
map_study_field_to_category <- function(field) {
  field <- tolower(field)
  if (grepl("physics|chemistry|biology|biomedicine|environmental|geology|astronomy|genetics|anthropology|physiology|earth science|engineering|informatics|data science|computer science|math|statistics|technology|informatics|it|information technology|artificial intelligence|science|health science|Medicine|baug|Eletrical engieneering|informatik|Geomatics|information system|medicine|geography", field)) {
    return("Natural Sciences")
  } else if (grepl("linguistics|literature|history|media|art|theater|music|philosophy|education|Geography and English|Japanese", field)) {
    return("Humanities")
  } else if (grepl("economics|finance|banking|management|law|political science|psychology|sociology|social work|social science|communication|international studies|cultural anthropology|gender studies|political science and economics|policy|Volkswirtschaftslehre|", field)) {
...
df_soc_learn_4round_zurich$study_subject_category <- sapply(df_soc_learn_4round_zurich$study_subject, map_study_field_to_category)

# Convert the new column to a factor
df_soc_learn_4round_zurich$study_subject_category <- factor(df_soc_learn_4round_zurich$study_subject_category)

# Compare number of observations per subject field:
sum(df_soc_learn_4round_zurich$study_subject_category=="Other")
sum(df_soc_learn_4round_zurich$study_subject_category=="Humanities")
sum(df_soc_learn_4round_zurich$study_subject_category=="Natural Sciences")
sum(df_soc_learn_4round_zurich$study_subject_category=="Social Sciences")

# Check which fields fall under "Other":
study_fields <- c("economics", "data science", "Physics", "Biomedicine", "Environmental engg", "Statistics", 
                  "Computer Science", "law", "computational biology", "Environmental Sciences", "mathematics", 
                  "Law", "computational linguistics", "Neurosciences", "environmental studies", "Architecture", 
                  "environmental engineering", "Mechanical Engineering", "Biology", "envirromental science", 
                  "Data Science", "Management", "Environmental Science", "Biomedical Engineering", "Math", 
                  "Environmental Engineering", "BIT", "physics", "Social science", "Economics", "Agrarian Sciences", 
                  "Health Sciences BSc.", "computer science", "Japanese Studies", "biology", "environmental science", 
                  "Volkswirtschaftslehre", "Computational Linguistics", "Philosophy", "communication", "Literature", 
                  "Neuroscience", "agricultural sciences", "Information Technology", "Health Science and Technology", 
                  "History and Philosophy", "Engineering", "history", "informatics", "Banking and Finance", "Food Science", 
                  "Computer science", "Chemistry", "health sciences", "Sociology", "Medicine", "biomedical engineering", 
                  "Mathematik", "Mathematics", "earth science ", "banking and finance", "Interdisciplinary Sciences", 
                  "art", "baug", "Health Sciences and technology", "Chemical engineering", "Political science", "finance", 
                  "Psychology and Linguistics", "medical sciences", "psychology", "Communication", "Eletrical engieneering", 
                  "science, technology and policy", "Politics", "material science", "Mechanical engineering", "engineering", 
                  "informatik", "architecture", "Environmental sciences", "interdisciplinary natural sciences", "D-ITET Bsc.", 
                  "Banking & Finance", "Artificial Intelligence", "Pharmaceutical sciences", "political science and economics", 
                  "Geography and English", "math", "health sciences and technology", "Geomatics", "Electrical Engineering", 
                  "emvironmental sciences", "information system", "History", "economics and management", "medicine", 
                  "communications", "food science", "Health Science & Technology", "Informatics", "mechanical engineering", 
                  "electrical engineering", "international and comparative law", "geography", "quantitative finance", 
                  "biomedicine", "Earth Sciences", "Finance", "Psychology", "data science and economics")

# Apply the categorization function
categorized_fields <- sapply(study_fields, map_study_field_to_category)

# Identify fields categorized as "Other"
other_fields <- study_fields[categorized_fields == "Other"]
...
df_soc_learn_4round_zurich$similar_ingroup_ncl_dummy <- as.factor(ifelse(df_soc_learn_4round_zurich$similar_ingroup == 1 & df_soc_learn_4round_zurich$observing_ingroup == 1 & df_soc_learn_4round_zurich$cognitive_load == 0, 1, 0))

# dissimilar to ingroup, observing the ingroup, no cognitive load
df_soc_learn_4round_zurich$dissimilar_ingroup_ncl_dummy <- as.factor(ifelse(df_soc_learn_4round_zurich$similar_ingroup == 0 & df_soc_learn_4round_zurich$observing_ingroup == 1 & df_soc_learn_4round_zurich$cognitive_load == 0, 1, 0))

# similar to ingroup, observing the outgroup, no cognitive load
df_soc_learn_4round_zurich$similar_outgroup_ncl_dummy <- as.factor(ifelse(df_soc_learn_4round_zurich$similar_ingroup == 1 & df_soc_learn_4round_zurich$observing_ingroup == 0 & df_soc_learn_4round_zurich$cognitive_load == 0, 1, 0))

# dissimilar to ingroup, observing the outgroup, no cognitive load
df_soc_learn_4round_zurich$dissimilar_outgroup_ncl_dummy <- as.factor(ifelse(df_soc_learn_4round_zurich$similar_ingroup == 0 & df_soc_learn_4round_zurich$observing_ingroup == 0 & df_soc_learn_4round_zurich$cognitive_load == 0, 1, 0))

# similar to ingroup, observing the ingroup, cognitive load
df_soc_learn_4round_zurich$similar_ingroup_cl_dummy <- as.factor(ifelse(df_soc_learn_4round_zurich$similar_ingroup == 1 & df_soc_learn_4round_zurich$observing_ingroup == 1 & df_soc_learn_4round_zurich$cognitive_load == 1, 1, 0))

# dissimilar to ingroup, observing the ingroup, cognitive load
df_soc_learn_4round_zurich$dissimilar_ingroup_cl_dummy <- as.factor(ifelse(df_soc_learn_4round_zurich$similar_ingroup == 0 & df_soc_learn_4round_zurich$observing_ingroup == 1 & df_soc_learn_4round_zurich$cognitive_load == 1, 1, 0))

# similar to ingroup, observing the outgroup, cognitive load
df_soc_learn_4round_zurich$similar_outgroup_cl_dummy <- as.factor(ifelse(df_soc_learn_4round_zurich$similar_ingroup == 1 & df_soc_learn_4round_zurich$observing_ingroup == 0 & df_soc_learn_4round_zurich$cognitive_load == 1, 1, 0))

# dissimilar to ingroup, observing the outgroup, cognitive load
df_soc_learn_4round_zurich$dissimilar_outgroup_cl_dummy <- as.factor(ifelse(df_soc_learn_4round_zurich$similar_ingroup == 0 & df_soc_learn_4round_zurich$observing_ingroup == 0 & df_soc_learn_4round_zurich$cognitive_load == 1, 1, 0))
...
df_soc_learn_no_cl <- subset(df_soc_learn_4round_zurich, df_soc_learn_4round_zurich$cognitive_load == 0)
...
df_soc_learn_cl <- subset(df_soc_learn_4round_zurich, df_soc_learn_4round_zurich$cognitive_load == 1)
...
#############################################################################
#############################################################################
# Regressions with omitted category
#############################################################################
#############################################################################
...
ols_optimal_choice <- glm(chosen_correctly ~ propIndCorrCentered + similar_ingroup_ncl_dummy + 
                         ...
                         data = df_soc_learn_4round_zurich, family = "gaussian")
...
# We reject the null that there is homoscedasticity. 
# We use cluster robust standard errors clustered on social learners

ols_optimal_choice_robust <- coeftest(ols_optimal_choice,vcov = vcovCL, cluster = df_soc_learn_4round_zurich$participant_ID)
...
# Test for normality of residuals (relevant for inference. use shapiro-wilk test):
...
cat("do SL pick their optimal urn", logit_optimal_choice, file = "logit_optimal_choice_zurich.txt", sep = "n", fill = TRUE, append = TRUE)
...
logit_optimal_choice_robust <- coeftest(logit_optimal_choice,vcov = vcovCL, cluster = df_soc_learn_4round_zurich$participant_ID)
...
cat("do SL pick their optimal urn, clustered SE", logit_optimal_choice_robust, file = "log_opt_robust_zurich.txt", sep = "n", fill = TRUE, append = F)
...
clustered_se <- sqrt(diag(vcovCL(logit_optimal_choice, cluster = df_soc_learn_4round_zurich$participant_ID)))
...
odd_ratio_opt <- ggplot(clustered_plot_data, aes(x = Variable, y = OddsRatio, ymin = LowerCI, ymax = UpperCI)) +
  ...
  scale_y_log10() +
  coord_flip() +
  ylab("Odds ratios (Zurich)") +
  xlab("") +
  theme_bw() +
  geom_hline(yintercept = 1, linetype = "dashed", color = "red")

# Save the plot
ggsave("odd_ratio_opt_zurich.png", plot = odd_ratio_opt, width = 8, height = 6, dpi = 300)
...
logit_optimal_choice_int_robust <- coeftest(logit_optimal_choice_int,vcov = vcovCL, cluster = df_soc_learn_4round_zurich$participant_ID)
...
cat("do SL pick their optimal urn, clustered SE", logit_optimal_choice_int_robust, file = "log_opt_interact_robust_zurich.txt", sep = "n", fill = TRUE, append = F)
...
cat("do SL pick their optimal urn", logit_optimal_choice_controls, file = "logit_optimal_choice_controls_zurich.txt", sep = "n", fill = TRUE, append = TRUE)
...
logit_optimal_choice_controls_robust <- coeftest(logit_optimal_choice_controls,vcov = vcovCL, cluster = df_soc_learn_4round_zurich$participant_ID)
...
cat("do SL pick their optimal urn, clustered SE", logit_optimal_choice_controls_robust, file = "logit_optimal_choice_controls_robust_zurich.txt", sep = "n", fill = TRUE, append = F)
...
cat("do SL pick their optimal urn", logit_optimal_choice_blockID, file = "logit_optimal_choice_blockID_zurich.txt", sep = "n", fill = TRUE, append = TRUE)
...
logit_optimal_choice_blockID_robust <- coeftest(logit_optimal_choice_blockID,vcov = vcovCL, cluster = df_soc_learn_4round_zurich$participant_ID)
...
cat("do SL pick their optimal urn, clustered SE", logit_optimal_choice_blockID_robust, file = "logit_optimal_choice_blockID_robust_zurich.txt", sep = "n", fill = TRUE, append = F)
...
# df_soc_learn_4round_zurich$uniqueSubjectNum <- rep(NA, length(df_soc_learn_4round_zurich[,1]))
# 
# id_integer <- 1
# 
# for (id in unique(df_soc_learn_4round_zurich$participant_ID)) {
#   
#   df_soc_learn_4round_zurich[which(df_soc_learn_4round_zurich$participant_ID==id),"uniqueSubjectNum"] <- id_integer
#   
...
# 
# df_soc_learn_4round_zurich$treatment_assignment <- df_soc_learn_4round_zurich$treatment_assignment + 1
# 
# # Now transform values such that everyone has values between 1-24:
# 
# for (i in 1:length(df_soc_learn_4round_zurich$treatment_assignment)) {
#   
#   # Use modulo operation and adjust for values that are multiples of 24
#   df_soc_learn_4round_zurich$treatment_assignment[i] <- df_soc_learn_4round_zurich$treatment_assignment[i] %% 24
#   if (df_soc_learn_4round_zurich$treatment_assignment[i] == 0) {
#     df_soc_learn_4round_zurich$treatment_assignment[i] <- 24
#   }
#   
# }
# 
# df_soc_learn_4round_zurich$treatment_assignment
# 
# 
# sum(df_soc_learn_4round_zurich$treatment_assignment==1)
# 
# 
# # Define the list of all permutations
# all_permutations <- list(
#   list(c(0.9, 1), c(0.9, 0), c(0.1, 1), c(0.1, 0)),
#   list(c(0.9, 1), c(0.9, 0), c(0.1, 0), c(0.1, 1)),
#   list(c(0.9, 1), c(0.1, 1), c(0.9, 0), c(0.1, 0)),
#   list(c(0.9, 1), c(0.1, 1), c(0.1, 0), c(0.9, 0)),
#   list(c(0.9, 1), c(0.1, 0), c(0.9, 0), c(0.1, 1)),
#   list(c(0.9, 1), c(0.1, 0), c(0.1, 1), c(0.9, 0)),
#   list(c(0.9, 0), c(0.9, 1), c(0.1, 1), c(0.1, 0)),
#   list(c(0.9, 0), c(0.9, 1), c(0.1, 0), c(0.1, 1)),
#   list(c(0.9, 0), c(0.1, 1), c(0.9, 1), c(0.1, 0)),
#   list(c(0.9, 0), c(0.1, 1), c(0.1, 0), c(0.9, 1)),
#   list(c(0.9, 0), c(0.1, 0), c(0.9, 1), c(0.1, 1)),
#   list(c(0.9, 0), c(0.1, 0), c(0.1, 1), c(0.9, 1)),
#   list(c(0.1, 1), c(0.9, 1), c(0.9, 0), c(0.1, 0)),
#   list(c(0.1, 1), c(0.9, 1), c(0.1, 0), c(0.9, 0)),
#   list(c(0.1, 1), c(0.9, 0), c(0.9, 1), c(0.1, 0)),
#   list(c(0.1, 1), c(0.9, 0), c(0.1, 0), c(0.9, 1)),
#   list(c(0.1, 1), c(0.1, 0), c(0.9, 1), c(0.9, 0)),
#   list(c(0.1, 1), c(0.1, 0), c(0.9, 0), c(0.9, 1)),
#   list(c(0.1, 0), c(0.9, 1), c(0.9, 0), c(0.1, 1)),
#   list(c(0.1, 0), c(0.9, 1), c(0.1, 1), c(0.9, 0)),
#   list(c(0.1, 0), c(0.9, 0), c(0.9, 1), c(0.1, 1)),
#   list(c(0.1, 0), c(0.9, 0), c(0.1, 1), c(0.9, 1)),
#   list(c(0.1, 0), c(0.1, 1), c(0.9, 1), c(0.9, 0)),
#   list(c(0.1, 0), c(0.1, 1), c(0.9, 0), c(0.9, 1))
# )
# 
# # Access examples
# # 4th ordering, 2nd treatment combination, 1st value
# # all_permutations[[1]][[1]][2]
# 
# 
# 
# similar_ingroup_first
# similar_ingroup_second
# similar_ingroup_third
# similar_ingroup_fourth
# 
# dissimilar_ingroup_first
# dissimilar_ingroup_second
# dissimilar_ingroup_third
# dissimilar_ingroup_fourth
# 
# similar_outgroup_first
# similar_outgroup_second
# similar_outgroup_third
# similar_outgroup_fourth
# 
# dissimilar_outgroup_first
# dissimilar_outgroup_second
# dissimilar_outgroup_third
# dissimilar_outgroup_fourth
...
logit_urn_choice_robust <- coeftest(logit_urn_choice,vcov = vcovCL, cluster = df_soc_learn_4round_zurich$participant_ID)
...
clustered_se <- sqrt(diag(vcovCL(logit_urn_choice, cluster = df_soc_learn_4round_zurich$participant_ID)))
...
ggsave("odd_ratio_pure_zurich.png", plot = odd_ratio_pure, width = 8, height = 6, dpi = 300)
...
logit_urn_choice_controls_robust <- coeftest(logit_urn_choice_controls,vcov = vcovCL, cluster = df_soc_learn_4round_zurich$participant_ID)
...
df_soc_learn_4round_zurich$number_observed_IL_right_urn <- number_ind_learn_per_group -
  ifelse(df_soc_learn_4round_zurich$group_assignment=="square" & df_soc_learn_4round_zurich$observing_ingroup,df_soc_learn_4round_zurich$number_left_urn_square,df_soc_learn_4round_zurich$number_left_urn_triangle)

df_soc_learn_4round_zurich$number_observed_IL_right_urn <- number_ind_learn_per_group -
  ifelse(df_soc_learn_4round_zurich$group_assignment=="triangle" & df_soc_learn_4round_zurich$observing_ingroup,df_soc_learn_4round_zurich$number_left_urn_triangle,df_soc_learn_4round_zurich$number_left_urn_square)
...
save(df_soc_learn_4round_zurich, file = "df_soc_learn_4round_zurich.RData")
...
# Uses df_soc_learn_4round_zurich dataset, 9999 bootstrapped samples (to give 10,000 together with actual sample) and standard 0.05 alpha.
est_df_soc_learn_4 <- mybootfunct(df_soc_learn_4round_zurich, 9999, 0.05)

# Saves the output of this function data frame.
est_df_soc_learn_4_zurich <- as.data.frame(est_df_soc_learn_4)
save(est_df_soc_learn_4_zurich, file = "est_df_soc_learn_4_zurich.RData")
...
output_test <- coeftest(output,vcov = vcovCL, cluster = df_soc_learn_4round_zurich$participant_ID)
...
df_soc_learn_firstblock <- df_soc_learn_4round_zurich[df_soc_learn_4round_zurich$round_number == 4, ]
...
sum(df_soc_learn_4round_zurich$cognitive_load==1)
sum(df_soc_learn_4round_zurich$cognitive_load==0)

# Observations of 8 different treatments:

nr_cl_sim_in <-  sum(df_soc_learn_4round_zurich$cognitive_load==1 & df_soc_learn_4round_zurich$similar_ingroup==1 & df_soc_learn_4round_zurich$observing_ingroup==1)
nr_cl_sim_out <- sum(df_soc_learn_4round_zurich$cognitive_load==1 & df_soc_learn_4round_zurich$similar_ingroup==1 & df_soc_learn_4round_zurich$observing_ingroup==0)
nr_cl_dissim_in <- sum(df_soc_learn_4round_zurich$cognitive_load==1 & df_soc_learn_4round_zurich$similar_ingroup==0 & df_soc_learn_4round_zurich$observing_ingroup==1)
nr_cl_dissim_out <- sum(df_soc_learn_4round_zurich$cognitive_load==1 & df_soc_learn_4round_zurich$similar_ingroup==0 & df_soc_learn_4round_zurich$observing_ingroup==0)
nr_ncl_sim_in <- sum(df_soc_learn_4round_zurich$cognitive_load==0 & df_soc_learn_4round_zurich$similar_ingroup==1 & df_soc_learn_4round_zurich$observing_ingroup==1)
nr_ncl_sim_out <-  sum(df_soc_learn_4round_zurich$cognitive_load==0 & df_soc_learn_4round_zurich$similar_ingroup==1 & df_soc_learn_4round_zurich$observing_ingroup==0)
nr_ncl_dissim_in <- sum(df_soc_learn_4round_zurich$cognitive_load==0 & df_soc_learn_4round_zurich$similar_ingroup==0 & df_soc_learn_4round_zurich$observing_ingroup==1)
nr_ncl_dissim_out <- sum(df_soc_learn_4round_zurich$cognitive_load==0 & df_soc_learn_4round_zurich$similar_ingroup==0 & df_soc_learn_4round_zurich$observing_ingroup==0)
...
save(obs_per_tmt, file = "obs_per_tmt_zurich.RData")
...
average_shared_winning_color_similar <- rep(NA,length(unique(df_soc_learn_4round_zurich$participant_ID)))

for (i in 1:length(unique(df_soc_learn_4round_zurich$participant_ID))) {
  average_shared_winning_color_similar[i] <- sum(df_soc_learn_4round_zurich[which(df_soc_learn_4round_zurich$participant_ID==unique(df_soc_learn_4round_zurich$participant_ID)[i] & df_soc_learn_4round_zurich$similar_ingroup==1),"share_winning_color"]) / length(df_soc_learn_4round_zurich[which(df_soc_learn_4round_zurich$participant_ID==unique(df_soc_learn_4round_zurich$participant_ID)[i] & df_soc_learn_4round_zurich$similar_ingroup==1),"share_winning_color"])
}
...
average_shared_winning_color_dissimilar <- rep(NA,length(unique(df_soc_learn_4round_zurich$participant_ID)))

for (i in 1:length(unique(df_soc_learn_4round_zurich$participant_ID))) {
  average_shared_winning_color_dissimilar[i] <- sum(df_soc_learn_4round_zurich[which(df_soc_learn_4round_zurich$participant_ID==unique(df_soc_learn_4round_zurich$participant_ID)[i] & df_soc_learn_4round_zurich$similar_ingroup==0),"share_winning_color"]) / length(df_soc_learn_4round_zurich[which(df_soc_learn_4round_zurich$participant_ID==unique(df_soc_learn_4round_zurich$participant_ID)[i] & df_soc_learn_4round_zurich$similar_ingroup==0),"share_winning_color"])
}
...
df_soc_learn_80round_zurich <- subset(df_soc_learn, round_number == 80)

nrow(df_soc_learn_80round_zurich)

df_soc_learn_80round_zurich_cl <- subset(df_soc_learn_80round_zurich, cognitive_load == 1)
df_soc_learn_80round_zurich_ncl <- subset(df_soc_learn_80round_zurich, cognitive_load == 0)


mean(df_soc_learn_80round_zurich$q1)
sd(df_soc_learn_80round_zurich$q1)

mean(df_soc_learn_80round_zurich$q2)
sd(df_soc_learn_80round_zurich$q2)

mean(df_soc_learn_80round_zurich$q3)
sd(df_soc_learn_80round_zurich$q3)

mean(df_soc_learn_80round_zurich_cl$q4)
sd(df_soc_learn_80round_zurich_cl$q4)

mean(df_soc_learn_80round_zurich_ncl$q4nocl)
sd(df_soc_learn_80round_zurich_ncl$q4nocl)

mean(df_soc_learn_80round_zurich_cl$q5)
sd(df_soc_learn_80round_zurich_cl$q5)

mean(df_soc_learn_80round_zurich_ncl$q5nocl)
sd(df_soc_learn_80round_zurich_ncl$q5nocl)

hist(df_soc_learn_80round_zurich_ncl$q5nocl)

mean(df_soc_learn_80round_zurich_cl$q6)
sd(df_soc_learn_80round_zurich_cl$q6)

mean(df_soc_learn_80round_zurich_cl$q7)
sd(df_soc_learn_80round_zurich_cl$q7)

mean(df_soc_learn_80round_zurich_cl$q8)
sd(df_soc_learn_80round_zurich_cl$q8)
